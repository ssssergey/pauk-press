...
# JSON API Search
@app.route('/api_search/<word>', methods=['GET'])
def api_search(word):
    bunch = []
    bunch.append((("%" + word.lower() + "%"), ("%" + word.capitalize() + "%"), ("%" + word.upper() + "%")))
    args = []
    for pair in bunch:
        args.append(or_(Main.article_title.like(pair[0]),
                        Main.article_title.like(pair[1]),
                        Main.article_title.like(pair[2]),
                        Main.article_text.like(pair[0]),
                        Main.article_text.like(pair[1]),
                        Main.article_text.like(pair[2]),
                        ))
    ...
    for row in query:
        ...
        posts.append(new_row)
    return Response(json.dumps({'items': posts}, ensure_ascii=False), content_type='application/json; charset=utf-8',
                    status=200)
...
def news_search():
    if request.method == 'POST':
        ...
        for i in words:
            if i:
                bunch.append((("%" + i.lower() + "%"), ("%" + i.capitalize() + "%"), ("%" + i.upper() + "%")))
        args = []
        for pair in bunch:
            args.append(or_(Main.article_title.like(pair[0]),
                            Main.article_title.like(pair[1]),
        ...
        query = Main.query.filter(and_(*[i for i in args])).order_by("article_time desc").limit(1500).all()
        posts = []
        for row in query:
            new_row = [row.article_title, ConvertTimeFormat(row.article_time), row.rss_source, row.id]
            posts.append(new_row)
